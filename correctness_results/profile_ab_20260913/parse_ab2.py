#!/usr/bin/env python3
"""Parse the A/B + LWPF-profile run logs.

Layout produced by ab_run2.sh:
    <logs_root>/<cfg>_<dataset>_<mode>/{old_np,new_np,new_p,old_p}.run.log

Emits two TSVs (or to stdout):
  * stage2 summary  - paired old_np vs new_np stage2 (the A/B answer)
  * kernel cycles   - per-kernel LWPF CYCLE average for new_p / old_p
"""
import os
import re
import sys
...
def main():
    if len(sys.argv) < 2:
        sys.exit("usage: parse_ab2.py <logs_root> [out_dir]")
    root = sys.argv[1]
