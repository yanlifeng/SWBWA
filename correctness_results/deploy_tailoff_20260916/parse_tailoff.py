#!/usr/bin/env python3
"""tail-off 实验解析器：解析 small/big4 × A/B × rep 的 discard 日志。

用法: parse_tailoff.py <log_dir>
输出: 写 <log_dir>/tailoff_summary.tsv，并在 stdout 打印 A/B 对照与哈希一致性。
"""
import re
import sys
...
def main():
    ...
    cols = ["log", "dataset", "config", "rep", "cg_count", "mpi_chunks",
            "world_size", "hash_ranks", "hash_bytes", "hash_sum", "hash_xor",
    ...
    for ds in ("small", "big4"):
        ...
        tag = "一致" if (len(s) == 1 and len(x) == 1) else "不一致!"
